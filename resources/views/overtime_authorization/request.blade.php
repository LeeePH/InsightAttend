...
<div class="row ot-page">
    <div class="col-12 px-1">
        <div class="card">
            <div class="card-body">
                <h4 class="page-header-title"><i class="fa fa-clock-o mr-1"></i> Overtime Authorization Form</h4>
                <p class="page-subtitle">Submit overtime schedule and reason for approval.</p>
                ...
                <form method="POST" action="{{ route('overtime_authorization.storeRequest') }}" id="overtimeAuthForm">
                    @csrf
                    <input type="hidden" name="date_filed" value="{{ $today }}">

                    <div class="table-responsive">
                        <table class="table table-bordered" id="otRowsTable">
                            <thead>
                                <tr>
                                    <th>Date</th>
                                    <th>Work From</th>
                            ...
                            <tbody>
                                @php $oldEntries = old('entries', [['date' => '', 'work_from' => '', 'work_to' => '', 'ot_from' => '', 'ot_to' => '', 'total_hours' => '', 'reason' => '']]); @endphp
                                ...
                                <tr>
                                    ...
                                    <td><input type="time" class="form-control form-control-sm" name="entries[{{ $idx }}][ot_from]" value="{{ $entry['ot_from'] ?? '' }}" required></td>
                                    <td><input type="time" class="form-control form-control-sm" name="entries[{{ $idx }}][ot_to]" value="{{ $entry['ot_to'] ?? '' }}" required></td>
                    ...
                    <button type="button" class="btn theme-btn mb-3" id="addOtRow"><i class="fa fa-plus"></i> Add Row</button>

                    <div class="form-group mb-0">
                        <button type="submit" class="btn theme-btn"><i class="fa fa-paper-plane"></i> Submit Overtime Authorization</button>
                        <a href="{{ route('employee.dashboard') }}" class="btn theme-btn ml-2"><i class="fa fa-home"></i> Back to Dashboard</a>
                    </div>
                </form>

                <div class="mt-4">
                    <h5 class="page-header-title mb-3">My Overtime Authorization Requests</h5>
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('overtime_authorization.show', $item->id) }}" class="btn theme-btn btn-sm"><i class="fa fa-eye"></i> View</a>
                                    </td>
                                </tr>
                                @empty
                                <tr><td colspan="4" class="text-center">No overtime authorization forms submitted yet</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
...
<script>
    (function () {
        var tableBody = document.querySelector('#otRowsTable tbody');
        var addBtn = document.getElementById('addOtRow');
        ...
        function buildRow(index) {
            return '<tr>'
                ...
                + '<td><input type="time" class="form-control form-control-sm" name="entries[' + index + '][ot_from]" required></td>'
                + '<td><input type="time" class="form-control form-control-sm" name="entries[' + index + '][ot_to]" required></td>'
